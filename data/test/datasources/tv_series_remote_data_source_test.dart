...
import 'package:data/models/tv_series_details_response.dart';
...
void main() {
  ...
  group('get Now Airing TV Series', () {
    ...
    test(
      ...
      () async {
        // arrange
        when(
          mockIOClient.get(Uri.parse('$baseUrl/tv/on_the_air?$apiKey')),
        ).thenAnswer(
          (_) async => http.Response(
            readJson('dummy_data/now_airing_tv_series.json'),
            200,
          ),
        );
        // act
  ...
  group('get tv series detail', () {
    ...
    test(
      ...
      () async {
        // arrange
        when(
          mockIOClient.get(Uri.parse('$baseUrl/tv/$tId?$apiKey')),
        ).thenAnswer(
          (_) async =>
              http.Response(readJson('dummy_data/tv_series_details.json'), 200),
        );
        // act
  ...
  group('search tv series', () {
    final tSearchResult =
        TvSeriesResponse.fromJson(
          json.decode(readJson('dummy_data/search_wandavision_tv_series.json')),
        ).tvSeriesList;
    ...
    test('should return list of tv series when response code is 200', () async {
      // arrange
      when(
        mockIOClient.get(Uri.parse('$baseUrl/search/tv?$apiKey&query=$tQuery')),
      ).thenAnswer(
        (_) async => http.Response(
          readJson('dummy_data/search_wandavision_tv_series.json'),
          200,
        ),
      );
      // act
